...
{
	{
		cout << __func__ << "::waiting for the mutex!" << endl;
		std::lock_guard<std::mutex> lck(mtx);	// 进入作用域, 构造函数获得mtx锁
		cout << __func__ << "::get the mutex! thread func be called! a: " << a << " thread id: " << std::this_thread::get_id() << endl;
...
void testThreadFunc()
{
	std::vector<std::thread> threadVec;
	{
		...
		std::thread t1(func, 10);				// 创建线程t1	// 线程对象没办法直接push_back进容器;	需要使用std::move
